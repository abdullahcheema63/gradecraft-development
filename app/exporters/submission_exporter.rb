    ...
    CSV.generate do |csv|
      ...
      course.submissions.submitted.find_each(batch_size: 500) do |submission|
        csv << [
          ...
          submission.group.try(:name),
          find_team_name(submission),
          submission.text_comment,
          submission.created_at,
          submission.updated_at,
          submission.grade.try(:score),
          find_graded_by_name(submission.grade.try(:graded_by_id)),
          submission.grade.try(:feedback),
          submission.grade.try(:graded_at)
        ]
      end
    end
  end
