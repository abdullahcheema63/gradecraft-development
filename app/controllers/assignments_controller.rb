...
class AssignmentsController < ApplicationController
  ...
  def copy
    begin
      ...
      duplicated.copy_unlock_conditions(assignment) if assignment.has_unlock_condition?
  ...
  def export_structure
    ...
    respond_to do |format|
      format.csv { send_data AssignmentExporter.new(current_user, course, "#{root_url}").export, filename: "#{ course.name } #{ (term_for :assignment).titleize } Structure - #{ Date.today }.csv" }
    end
  end
